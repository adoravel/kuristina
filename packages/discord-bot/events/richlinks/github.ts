...
import { extractBlobRefs, fetchSnippet, renderSnippet } from "@kuristina/services/github";
...
export async function handleGitHubLinks(bot: typeof discord, message: Message): Promise<void> {
	...
	for (const ref of refs) {
		...
		await sendCompanion(bot, message, renderSnippet(ref, snippet.value), "richlink:github");
		sent++;
	}
