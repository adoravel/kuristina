...
import type { Input, ParsingResult } from "./mod.ts";

const UNSET = Symbol("combinators.unset");
...
export const boo = (
	...
	found: string,
	message?: string,
	contextFn?: () => ErrorContext | undefined,
	stack: ParserStackFrame[] = [],
): ParsingResult<never> => {
	const exp = expected.map((x) => x || "<anon>") as string[];
	let cachedMessage: string | typeof UNSET = message ?? UNSET;
	let cachedContext: ErrorContext | undefined | typeof UNSET = UNSET;

	const data: ParsingError = {
		position,
		expected,
		found,
		stack,
		get message() {
			if (cachedMessage === UNSET) cachedMessage = buildDefaultMessage(exp, found, position);
			return cachedMessage as string;
		},
		get context() {
			if (cachedContext === UNSET) cachedContext = contextFn?.();
			return cachedContext as ErrorContext | undefined;
		},
	} as ParsingError;
...
export function unexpectedSymbol(
	...
	return boo(getPosition(input), expected, found, undefined, () => getContext(input), stack);
}
