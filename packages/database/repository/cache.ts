...
export class ExternalCacheRepository extends Repository {
	async get<T>(key: string, ttlSeconds: number): Promise<Result<T | null, SqlError>> {
		return await tryQuery(async () => {
			const row = await this.database.selectFrom("external_cache")
				.select(["payload", "cached_at"]).where("cache_key", "=", key).executeTakeFirst();
			if (!row) return null;
	...
	async set<T>(key: string, value: T): Promise<Result<void, SqlError>> {
		return await tryQuery(() =>
			this.database.insertInto("external_cache")
				.values({
					cache_key: key,
	...
	async purgeExpired(ttlSeconds: number): Promise<Result<number, SqlError>> {
		return await tryQuery(async () => {
			...
			const result = await this.database.deleteFrom("external_cache")
